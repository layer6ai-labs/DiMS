...
def save_checkpoint(cfg: CheckpointConfig, trainer, epoch_itr, val_loss, step=""):
    ...
    prev_best = getattr(save_checkpoint, f"best{step}", val_loss)
    if val_loss is not None:
        best_function = max if cfg.maximize_best_checkpoint_metric else min
        setattr(save_checkpoint, f"best{step}", best_function(val_loss, prev_best))
    ...
    suffix = trainer.checkpoint_suffix + step
    ...
    checkpoint_conds["checkpoint_best{}.pt".format(suffix)] = val_loss is not None and (
        not hasattr(save_checkpoint, f"best{step}")
        or is_better(val_loss, getattr(save_checkpoint, f"best{step}"))
    )
    if val_loss is not None and cfg.keep_best_checkpoints > 0:
        worst_best = getattr(save_checkpoint, f"best{step}", None)
        ...
        if len(chkpts) > 0:
            ...
            worst_best = float(p.rsplit("_")[-1 if step=="" else -2].replace("{}.pt".format(suffix), ""))
    ...
    checkpoint_conds[
        "checkpoint_last{}.pt".format(trainer.checkpoint_suffix)
    ] = not cfg.no_last_checkpoints

    extra_state = {"train_iterator": epoch_itr.state_dict(), "val_loss": val_loss}
    if hasattr(save_checkpoint, f"best{step}"):
        extra_state.update({f"best{step}": getattr(save_checkpoint, f"best{step}")})
