...
class IterativeRefinementGenerator(object):
    def __init__(
        ...
        if blank_symbol is not None:
            self.blank=tgt_dict.index(blank_symbol)
        ...
        self.adaptive = False
        self.models = models
        self.bleu_reranker = bleu_reranker
        self.deduplicate=deduplicate
        if self.bleu_reranker:
            print("BLEU RERANKER IS ON!!!")
    ...
    @torch.no_grad()
    def bleu_rerank(self, finalized, tgt):
        scorer = bleu.Scorer(self)
        refs = tgt.unsqueeze(1).expand(-1, self.beam_size, -1).reshape(-1, tgt.shape[-1])
        for hyp,ref in zip(finalized, refs):
            ref = torch.IntTensor(ref.int().cpu())
            scorer.add(ref, torch.IntTensor(hyp[0]["tokens"].int().cpu()))
            hyp[0]['score'] = torch.tensor(scorer.score())
            scorer.reset()
        return finalized

    @torch.no_grad()
    def generate(self, models, sample, prefix_tokens=None, constraints=None, **kwargs):
        ...
        if not getattr(model.args, 'ctc', False) and self.beam_size > 1:
            assert (
        ...
        for step in range(self.max_iter+1):
            # step_coeff = torch.stack([(1+torch.randperm(5))/5 for _ in range(bsz // self.beam_size)]).reshape(-1).cuda()
            # if step==self.max_iter:
            #     step=torch.tensor([self.max_iter+1])
            # else:
            #     step = (step*torch.ones_like(step_coeff) + step_coeff[torch.randperm(len(step_coeff))]).cuda() 
            ...
            decoder_out, _ = model.forward_decoder(
        ...
        if self.deduplicate:
            for f in finalized:
                f[0]["tokens"] = f[0]["tokens"][torch.cat((f[0]["tokens"][:-1] != f[0]["tokens"][1:], torch.tensor([True]).cuda()))]
                if getattr(self, 'blank', None)!=None:
                    f[0]["tokens"][f[0]["tokens"]==self.blank]=self.bos

        
        if not getattr(model.args, 'ctc', False) and self.beam_size > 1:
            if self.bleu_reranker:
                finalized=self.bleu_rerank(finalized, sample['target'])
